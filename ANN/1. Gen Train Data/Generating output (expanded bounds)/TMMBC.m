function [isc, R_avg, T_avg] = TMMBC(x,xstepsize,L1,L2,L3,L4,L5,L6,L7,L8,L9,L10,L11,L12,L13,LglassTC)
...
% layers = {'SodaLime' 'ITO' 'NiO' 'PerovHMv2' 'C60HM' 'SnOxHM' 'ITO' 'LiF' 'Air'};
layers = {'SodaLime' 'ITO' 'NiO' 'PerovHMv2' 'C60HM' 'SnO2' 'ITO' 'LiF'...
    'MgF2' 'AZO' 'ZnO' 'CdS' 'CIS' 'Mo' 'SodaLime' 'Air'};
% layers = {'SodaLime' 'ITO' 'NiO' 'Perov' 'C60' 'BCP' 'ITO' 'LiF' 'Air'};
...
Gsumn = zeros(row,no_lambda);
lam = 300e-9;
AM15_count = 1;
R_BC = zeros(1,no_lambda);
T_TC = zeros(1,no_lambda);
...
% Whole Cell coherent scattering matrix
    M = I_0*L_1*I_1*L_2*I_2*L_3*I_3*L_4*I_4*L_5*I_5*L_6*I_6*L_7*I_7*L_8*I_8*L_9*I_9*L_10*I_10*L_11*I_11*L_12*I_12*L_13*I_13;
    M11(1,i)=M(1,1);
    M12(1,i)=M(1,2);
    M21(1,i)=M(2,1);
    M22(1,i)=M(2,2);
    ...
    % Top Cell coherent scattering matrix
    I_7_T = I_mat(n(8,i),n(end,i));
    M_T = I_0*L_1*I_1*L_2*I_2*L_3*I_3*L_4*I_4*L_5*I_5*L_6*I_6*L_7*I_7_T;
    R12_T=abs(M_T(2,1)/M_T(1,1))^2;
    T12_T=abs(1/M_T(1,1))^2;
    R21_T=abs(M_T(1,2)/M_T(1,1))^2;
    T21_T=abs(M_T(2,2)-(M_T(2,1)*M_T(1,2)/M_T(1,1)))^2;

    % Bottom Cell coherent scattering matrix
    I_7B = I_mat(n(end,i),n(9,i));
    M_B = I_7*L_8*I_8*L_9*I_9*L_10*I_10*L_11*I_11*L_12*I_12*L_13*I_13;
    R12_B=abs(M_B(2,1)/M_B(1,1))^2;
    T12_B=abs(1/M_B(1,1))^2;
    R21_B=abs(M_B(1,2)/M_B(1,1))^2;
    T21_B=abs(M_B(2,2)-(M_B(2,1)*M_B(1,2)/M_B(1,1)))^2;
...
%     E1f_sq = abs(sqrt((2*AM15(1,i))/(c*epsilon0)))^2;
    ...
    T_TC(1,i) = -(P1*T01*T12_T)/(- P1^2 + R10*R12_T);    % Transmittance of TC to BC
    R_BC(1,i) = R12_B;                               % Reflectance of BC to TC
...
% Top cell electric field calculation
    ...
    Mf = I_0*L_1*I_1*L_2*I_2*L_3*I_3*L_4*I_4*L_5*I_5*L_6*I_6*L_7*I_7*...
        L_8*I_8*L_9*I_9*L_10*I_10*L_11*I_11;
    Mb = L_12*I_12*L_13*I_13;
...
%     Sb22 = Mb(2,2);    
    ...
    v11f = ((Sb11/(Sb11*Sf11+Sf12*Sb21)))*Ef;
    w11f = ((Sb21/(Sb11*Sf11+Sf12*Sb21)))*Ef;
    
    k_CIS = (2*pi/lam)*n(13,i);
    nk_CIS = real(n(13,i))*imag(n(13,i));
    
    EzF(:,1) = v11f.*exp(1i.*k_CIS.*x(:,1))+w11f.*exp(-1i.*k_CIS.*x(:,1));
    
    Ez_mag(:,1) = abs(EzF(:,1)).^2;
    
    Gsumn(:,i) = ((2*pi*nk_CIS)/(h))*epsilon0*Ez_mag(:,1);
%     GsumnTC(:,i) = 10*AM15(1,AM15_count)*(4*pi*nk_perovTC/(h*c))*Ez_mag(:,1);
    EQE(1,i) = sum(Gsumn(:,i).*h.*c./lambda(1,i)./AM15(1,i));
...
f_odd = zeros(row,1);
    ...
    for j = 2:1:no_lambda-1
        if mod(j,2)==0
            f_odd(:,1) = f_odd(:,1)+Gsumn(:,j);
        else
            f_even(:,1) = f_even(:,1)+Gsumn(:,j);
        end
    end
    Gx(:,1) = (stepsize/3)*(f_0+4.*f_odd+2.*f_even+f_n);
    isc=sum(Gx(:,1))*(xstepsize)*q;
    T_avg = mean(T_TC(1,200:end));
    R_avg = mean(R_BC(1,1:500));
end
